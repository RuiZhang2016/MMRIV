...
def test_LMO_err(sname, seed,use_x_images,use_z_images, nystr=True):
    ...
    def LMO_err(params,M=2,verbal=False):
        ...
        # L_inv = chol_inv(L)
        tmp_mat = L
        C = L-tmp_mat@np.linalg.inv(tmp_mat/N2+inv_eig_val)@tmp_mat.T/N2
        c = C@W_nystr_Y*N2
        c_y = c-Y
        lmo_err = 0
        N = 0
    ...
    def callback0(params):
        ...
        if Nfeval % 1 == 0:
            ...
            if nystr:
                tmp_mat = L
                alpha = EYEN-np.linalg.inv(tmp_mat/N2+inv_eig_val)@tmp_mat/N2
    ...
    Y = np.vstack((train.y,dev.y))[:1000]
    Z = np.vstack((train.z,dev.z))[:1000]
