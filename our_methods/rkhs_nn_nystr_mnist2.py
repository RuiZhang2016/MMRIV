...
def run_experiment_nn(scenario_name,indices=[],seed=527,training=True):
    ...
    def fit(x,y,z,lr,decay_weight,n_epochs=n_epochs):
        ...
        optimizer = optim.Adam([alpha], lr=lr, weight_decay=decay_weight)
        for epoch in range(n_epochs):
            permutation = torch.randperm(n_data)

            for i in range(0, n_data, batch_size):
                indices = permutation[i:i + batch_size]
                batch_x, batch_y, batch_z = test_X[indices], test_G[indices], z[indices]
                N2 = torch.tensor(len(batch_y)**2).float()
                ak = get_median_inter_mnist(batch_z)
                W = (k(batch_z,None,ak,1)+k(batch_z,None,ak*10,1)+k(batch_z,None,ak/10,1))/3
                W /= N2
                # training loop
                def closure():
                    optimizer.zero_grad()
                    feature_bx = batch_x # net1(batch_x)
                    feature_x = test_X # net1(x)
                    L = l(feature_x,feature_x,al,bl)
                    test_L = l(feature_bx,feature_x,al,bl)
                    # loss1 = my_loss(batch_y,L,alpha, W)
                    # pred_y= net2(feature_x)
                    #loss2 =  (batch_y-L@alpha).t()@W@(batch_y-L@alpha)
                    #print(i,feature_x,loss2)
                    test_error = test_err(batch_y,L,test_L,alpha)
