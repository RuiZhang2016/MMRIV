...
def test_LMO_err(i,j, seed=527, nystr=True):
    ...
    def LMO_err(params,M=2,verbal=False):
        ...
        for i in range(0, X.shape[0], M):
            ...
            c_y_i = c_y[indices]
            I_CK = EYEM-C_i@K_i
            lmo_err1 += c_y_i.T@np.linalg.inv(I_CK.T)@K_i@c_y_i
            (_,val) = np.linalg.slogdet(I_CK)
            lmo_err2 -= val
            N += 1
        return (lmo_err1[0,0]+lmo_err2[0,0])/N/M**2
    ...
    def callback0(params):
        ...
        if Nfeval % 1 == 0:
            ...
            plt.legend()
            plt.savefig(ROOT_PATH + '/our_methods/leave_{}_m_err_{}_{}_2.pdf'.format(sname,i,j))
            plt.close('all')
        Nfeval += 1
    ...
    #    if sid == 0:
            # X, Z = [np.vstack(e) for e in [(train.x, dev.x), (train.z, dev.z)]]
            # test_X = test.x
            # sorted_id = np.argsort(test_X,axis=0).flatten()
            # test_X = test_X[sorted_id]
            # ak = get_median_inter_mnist(Z)
            # W = k(Z,None,ak,1)/Z.shape[0]**2
            # N2 = X.shape[0]**2
            # permutation = np.random.permutation(X.shape[0])
    ...
    #        test_L = l(test_X,X,al,1)
        ...
        np.save(PATH+'LMO_errs_{}_{}_2.npy'.format(i,j),[res,norm,test_err])
...
def plot_res_2d(sname):        
    ...
    res = np.array([np.load(PATH+'LMO_errs_{}_{}_2.npy'.format(i,j),allow_pickle=True) for i in range(len(als)) for j in range(len(bls)) if os.path.exists(PATH+'LMO_errs_{}_{}_2.npy'.format(i,j))])
    res = np.array([[e[0], e[1], e[2]] for e in res])
    # res[:,[0]] += res[:,[-2]]
    ...
    for i in range(res.shape[1]-1):
        ...
        min_al,min_bl = divmod(min_ids[i],len(bls))
        ...
        min_al,min_bl = divmod(min_ids[-1],len(bls))
        al,bl = als[min_al],bls[min_bl]
        ax2.plot(bl,al,marker='o',color='r')
        plt.savefig('leave_M_{}_{}_2.pdf'.format(sname,i), bbox_inches='tight')
        plt.close('all')
